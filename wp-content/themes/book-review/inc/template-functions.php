 ...
 * @return array
 */
function bookreview_post_classes( $classes ) {
	$classes[] = 'entry';

	return $classes;
}
add_filter( 'post_class', 'bookreview_post_classes', 10, 3 );
 ...
 * @return void
 */
function bookreview_supports_js() {
	echo '<script>document.body.classList.remove("no-js");</script>';
}
add_action( 'wp_footer', 'bookreview_supports_js' );
 ...
 * @return array
 */
function bookreview_comment_form_defaults( $defaults ) {
...
add_filter( 'comment_form_defaults', 'bookreview_comment_form_defaults' );
 ...
 * @return bool
 */
function bookreview_can_show_post_thumbnail() {
	/**
	...
	return apply_filters(
		'bookreview_can_show_post_thumbnail',
 ...
 * @return int
 */
function bookreview_get_avatar_size() {
	return 60;
}
 ...
 * @since Book Review 1.0
 */
function bookreview_continue_reading_text() {
	$continue_reading = sprintf(
		/* translators: %s: Name of current post. */
		esc_html__( 'Continue reading %s', 'bookreview' ),
 ...
 * @since Book Review 1.0
 */
function bookreview_continue_reading_link_excerpt() {
	if ( ! is_admin() ) {
		return '&hellip; <a class="more-link" href="' . esc_url( get_permalink() ) . '">' . bookreview_continue_reading_text() . '</a>';
	}
}

// Filter the excerpt more link.
add_filter( 'excerpt_more', 'bookreview_continue_reading_link_excerpt' );
 ...
 * @since Book Review 1.0
 */
function bookreview_continue_reading_link() {
	if ( ! is_admin() ) {
		return '<div class="more-link-container"><a class="more-link" href="' . esc_url( get_permalink() ) . '#more-' . esc_attr( get_the_ID() ) . '">' . bookreview_continue_reading_text() . '</a></div>';
	}
}

// Filter the excerpt more link.
add_filter( 'the_content_more_link', 'bookreview_continue_reading_link' );

if ( ! function_exists( 'bookreview_post_title' ) ) {
	/**
	 ...
	 * @return string
	 */
	function bookreview_post_title( $title ) {
		return '' === $title ? esc_html_x( 'Untitled', 'Added to posts and pages that are missing titles', 'bookreview' ) : $title;
	}
}
add_filter( 'the_title', 'bookreview_post_title' );
 ...
 * @return string
 */
function bookreview_get_icon_svg( $group, $icon, $size = 24 ) {
	return Book_Review_SVG_Icons::get_svg( $group, $icon, $size );
}
 ...
 * @return string
 */
function bookreview_change_calendar_nav_arrows( $calendar_output ) {
	$calendar_output = str_replace( '&laquo; ', is_rtl() ? bookreview_get_icon_svg( 'ui', 'arrow_right' ) : bookreview_get_icon_svg( 'ui', 'arrow_left' ), $calendar_output );
	$calendar_output = str_replace( ' &raquo;', is_rtl() ? bookreview_get_icon_svg( 'ui', 'arrow_left' ) : bookreview_get_icon_svg( 'ui', 'arrow_right' ), $calendar_output );
	return $calendar_output;
}
add_filter( 'get_calendar', 'bookreview_change_calendar_nav_arrows' );
 ...
 * @return string
 */
function bookreview_get_non_latin_css( $type = 'front-end' ) {
	...
	$font_family = apply_filters(
		'bookreview_get_localized_font_family_types',
		array(
	...
	$elements = apply_filters(
		'bookreview_get_localized_font_family_elements',
		array(
	...
	if ( ! function_exists( 'bookreview_generate_css' ) ) {
	...
	return bookreview_generate_css( // @phpstan-ignore-line.
 ...
 */
function bookreview_print_first_instance_of_block( $block_name, $content = null, $instances = 1 ) {
	$instances_count = 0;
	$blocks_content  = '';
 ...
 *
 * @since Book Review 1.0
 * @since Book Review 1.4 Corrected parameter name for `$output`,
 ...
 */
function bookreview_password_form( $output, $post = 0 ) {
	$post   = get_post( $post );
	$label  = 'pwbox-' . ( empty( $post->ID ) ? wp_rand() : $post->ID );
	$output = '<p class="post-password-message">' . esc_html__( 'This content is password protected. Please enter a password to view.', 'bookreview' ) . '</p>
	<form action="' . esc_url( site_url( 'wp-login.php?action=postpass', 'login_post' ) ) . '" class="post-password-form" method="post">
	<label class="post-password-form__label" for="' . esc_attr( $label ) . '">' . esc_html_x( 'Password', 'Post password form', 'bookreview' ) . '</label><input class="post-password-form__input" name="post_password" id="' . esc_attr( $label ) . '" type="password" size="20" /><input type="submit" class="post-password-form__submit" name="' . esc_attr_x( 'Submit', 'Post password form', 'bookreview' ) . '" value="' . esc_attr_x( 'Enter', 'Post password form', 'bookreview' ) . '" /></form>
	';
	return $output;
}
add_filter( 'the_password_form', 'bookreview_password_form', 10, 2 );
 ...
 */
function bookreview_get_attachment_image_attributes( $attr, $attachment, $size ) {
...
add_filter( 'wp_get_attachment_image_attributes', 'bookreview_get_attachment_image_attributes', 10, 3 );
