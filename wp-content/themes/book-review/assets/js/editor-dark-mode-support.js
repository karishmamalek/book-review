/* global bookreviewIsDarkMode, setTimeout */

// Check the color scheme preference and inject the classes if necessary.
if ( document.body.classList.contains( 'bookreview-supports-dark-theme' ) ) {
	bookreviewDarkModeEditorInit();
}
 ...
 * @return {void}
 */
function bookreviewDarkModeEditorInit( attempt ) {
	...
	if ( bookreviewIsDarkMode() ) {
		if ( null === container ) {
			// Try again.
			if ( attempt < maxAttempts ) {
				setTimeout(
					function() {
						bookreviewDarkModeEditorInit( attempt + 1 );
					},
